...
class miband(Peripheral):
    ...
    GYROSCOPE_SERVICE_UUID = "0000180d-0000-1000-8000-00805f9b34fb"  # Replace with the actual UUID
    GYROSCOPE_CHARACTERISTIC_UUID = "00002a37-0000-1000-8000-00805f9b34fb"  # Replace with the actual UUID
    ...
    def __init__(self, mac_address,key=None, timeout=0.5, debug=False):
        ...
        self.svc_gyroscope = self.getServiceByUUID(GYROSCOPE_SERVICE_UUID)
        self._char_gyroscope = self.svc_gyroscope.getCharacteristics(GYROSCOPE_CHARACTERISTIC_UUID)[0]
    ...
    def start_gyroscope_realtime(self, callback):

        print("Initializing gyroscope streaming...")

        # Ensure connection
        if not self.is_connected:
            raise BTLEDisconnectError("Device not connected")

        # Enable notifications for the gyroscope characteristic
        self._char_gyroscope.write(b'\x01\x00', withResponse=True)

        # Subscribe to gyroscope characteristic
        def handle_gyroscope_notification(handle, data):
            # Parse data (example assumes 6 bytes, 2 bytes each for x, y, z)
            if len(data) == 6:
                ...
                print("Unexpected gyroscope data format")

        self.setDelegate(DefaultDelegate(handleNotification=handle_gyroscope_notification))

        print("Gyroscope notifications enabled. Waiting for data...")
        ...
        try:
            ...
            # Stop notifications and handle cleanup if necessary
            self._char_gyroscope.write(b'\x00\x00', withResponse=True)
            print("\nStopped gyroscope data stream.")
